...
class uploadtoyseclass(downloadlcloopclass,autoaddclass):
    ...
    def uploadtoyse(self,filename):
        print('python %s/uploadTransientData.py -e -s %s/settings.ini -i %s --instrument ACAM1 --fluxzpt 27.5' % (self.sourcedir,self.sourcedir,filename))
    ...
    def averageyselc(self,args,TNSname,filt,MJDbinsize=1.0):
        ...
        ### DELME!!!
        #MJD=59344.1
        print('BBBBBBBBBBBBBBBBBBBBBBBBBBB',MJD,MJDmax)
        while MJD <= MJDmax:
            ...
            self.lc.write(indices=mjd_ix)
            
            # sigmacut indices
            self.lc.calcaverage_sigmacutloop(self.flux_colname,noisecol=self.dflux_colname,indices=mjd_ix,verbose=3,Nsigma=3.0,median_firstiteration=True)
            fluxstatparams = deepcopy(self.lc.statparams)
            print('FLUX MEAN:',self.lc.statstring())

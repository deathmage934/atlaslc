...
class offsetstatsclass(SNloopclass):
	...
	def cutandaveragelc(self,SNindex,offsetindex=0,MJDbinsize=1):
		...
		self.averagelc.t.drop(self.averagelc.ix_inrange(lowlim=0,uplim=len(self.averagelc.t)))
		#self.averagelc.t[self.flux_colname] = pd.Series([], dtype=np.float64)
		...
		self.averagelc.t['Mask'] = pd.Series([], dtype=np.int64)

		self.averagelc.write()

		while MJD <= MJDmax:
			# get 4 measurements
			if self.verbose>2: print('MJD range: ',MJD,' to ',MJD+1)
			ix1 = self.lc.ix_inrange(colnames=['MJD'],lowlim=MJD,uplim=MJD+MJDbinsize,exclude_uplim=True)
			if len(ix1)==0:
				if self.verbose>2: print('Length of MJD range = 0, skipping MJD range...')
				MJD += MJDbinsize
				continue
			else:
				if self.verbose>2: self.lc.write(indices=ix1) # delete me
			...
			if len(ix2)==0:
				if self.verbose>2: print('Length of good and ok measurements = 0, skipping MJD range...')
				MJD += MJDbinsize
				continue
			else:
				if self.verbose>2: print('Good and ok indices (ix2): ',ix2)
			...
			if fluxstatparams['mean'] is None or len(fluxstatparams['ix_good'])<1:
				if self.verbose>2: print('Mean uJy is None OR length index good < 1, flagging bad day and skipping MJD range...')
				self.lc.t.loc[ix1,'Mask'] = np.bitwise_or(self.lc.t.loc[ix1,'Mask'],self.flag_badday)
				continue

			# get average mjd
			self.lc.calcaverage_sigmacutloop('MJD',noisecol='duJy',indices=fluxstatparams['ix_good'],verbose=2,Nsigma=0,median_firstiteration=False)
			averagemjd = self.lc.statparams['mean']
			print('Average MJD: ',averagemjd,' (check MJD range: ',MJD,' to ',MJD+1,')')

			# construct averagelc table
			...
			#self.averagelc.t.at[lcaverageindex,'MJD'] = averagemjd
			#self.averagelc.t.loc[lcaverageindex,self.flux_colname] = fluxstatparams['mean']
			#self.averagelc.t.loc[lcaverageindex,self.dflux_colname] = fluxstatparams['mean_err']
			#self.averagelc.t.at[lcaverageindex,'stdev'] = fluxstatparams['stdev']
			#self.averagelc.t.at[lcaverageindex,'X2norm'] = fluxstatparams['X2norm']
			#self.averagelc.t.at[lcaverageindex,'Nclip'] = fluxstatparams['Nclip']
			#self.averagelc.t.at[lcaverageindex,'Ngood'] = fluxstatparams['Ngood']
			#self.averagelc.t.at[lcaverageindex,'Nexcluded'] = len(ix1) - len(ix2)
			#self.averagelc.t.at[lcaverageindex,'Mask'] = 0
			if self.verbose>2: print(self.averagelc.t) 
	...
	def offsetstatsloop(self,SNindex,filt):
		...
		for offsetindex in range(1,len(self.RADECtable.t)):
			...
			if (len(self.lc.t) != N_MJD) or (np.array_equal(MJD_SN, self.lc.t['MJD']) is False):
				if self.verbose>1: print(MJD_SN,'\n',self.lc.t['MJD'])
